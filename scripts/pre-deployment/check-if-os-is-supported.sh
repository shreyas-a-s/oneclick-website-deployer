...
if command -v whiptail > /dev/null; then # Display messages using whiptail
  if ! command -v apt-get > /dev/null; then # Checking is OS is not Debian-based
    if ! whiptail --title "Warning" --yesno --defaultno --yes-button "Continue" --no-button "Cancel" "This doesn't seem to be a Debian Linux system. This system is [$os_name]. Installation might not work properly. But you can continue with the installation if you want." 10 56; then
      exit 1
    fi
  elif [ "$debian_version" -ne 11 ]; then # Checking if OS is debian but not version 11
    if ! whiptail --title "Warning" --yesno --defaultno --yes-button "Continue" --no-button "Cancel" "Drupal 7 (and thereby Tripal 3) works best in Debian 11. This system is [Debian $debian_version]. Installation might not work properly in this system. But you can continue with the installation if you want." 10 56; then
      exit 1
    fi
  fi
