...
if command -v whiptail > /dev/null; then # Display messages using whiptail
  if ! command -v apt-get > /dev/null; then # Checking is OS is not Debian-based
    if ! whiptail --title "Warning" --yesno --yes-button "Continue" --no-button "Cancel" "Drupal 7 (and thereby Tripal 3) works best in Debian 11. This system is Debian $debian_version. Installation might not work properly in this system. But you can continue with the installation if you want." 10 56; then
      exit 1
    fi
